...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      parts: [{ text: "Hi there! I'm the CampusServe assistant. How can I help you today?" }],
        ...
        {!isOpen && (
          ...
          >
            <motion.button
              ...
              className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white p-3 rounded-full shadow-2xl hover:from-indigo-600 hover:to-pink-600 transition-all"
            >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed bottom-4 right-4 w-[450px] h-[600px] ${themeColors.container} rounded-2xl shadow-2xl border-4 border-opacity-50 border-purple-300 flex flex-col overflow-hidden`}
          >
            {/* Header */}
            <div className={`${themeColors.header} p-4 flex justify-between items-center`}>
              <div className="flex items-center space-x-2">
                <Sparkles className="text-yellow-300" size={24} />
                <h2 className="text-lg font-bold">CampusServe Assistant</h2>
              </div>
              <div className="flex items-center space-x-2">
                <motion.button
            ...
            <div className="p-4 border-t flex space-x-2">
              <input
                ...
                className="flex-1 p-3 border-2 border-purple-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
              />
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                ...
                disabled={isLoading}
                className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white p-3 rounded-xl hover:from-indigo-600 hover:to-pink-600 disabled:opacity-50 transition-all"
              >
                <Send size={20} />
              </motion.button>
            </div>
